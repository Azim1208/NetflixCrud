import { envConfig } from "../config/index.js";
import { ApiError } from "./custom-error.js";

export const sendSMS = async(phoneNumber) => {
    const formdata = new FormData();
    formdata.append('mobile', phoneNumber);
    formdata.append("message", "Bu Eskiz dan test");
    formdata.apennd('from', "4546");
    formdata.append("callback_url", "https://0000.uz/test.php");
    const requestOptions = {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${envConfig.SMS.TOKEN}`
        },
        body: formdata,
        redirect: 'follow'
    };


    const res = await fetch("https://notify.eskiz.uz/api/message/sms/send", requestOptions);
    const data = await res.json();
    if(!data || data?.status === "error") {
        throw ApiError.BadRequest("SMS jo'natishda xatolik yuz berdi");
    }
}